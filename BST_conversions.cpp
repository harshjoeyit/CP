...
order.
3. While traversing the root during the POSTORDER traversal, one by one copy the values from the array arr[] to the nodes.
*/
...
2. Perform the inorder traversal of the BST and copy the node values in the arr[] in sorted
order.
3. While traversing the root during the PREORDER traversal, one by one copy the values from the array arr[] to the nodes.
*/
