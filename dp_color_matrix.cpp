...
int dp[mxN][4][4][4];               // i, c1, c2, c3
int n;

int count_ways(int ind, int c1, int c2, int c3) {             // prev column and colors of each row c1 - 1, c2 - 2, c3 - 3
    if(ind == n)
        return 1;
    ...
    for(int i=1; i<=3; i++) {               
        if(i == c1)  continue;
        for(int j=1; j<=3; j++) {
            if( j == c2 || j == i)  continue;
            for(int k=1; k<=3; k++) {   
                if(k == c3 || k == j)  continue;
