...
int rangeSumQuery(int *tree, int index,int s, int e, int l, int r)
{
    //  1. no overlap
    if(l > e || r < s)
        return 0;
    ...
    // 3. partial overlap
    int mid = (s + e)/2;
    int leftans = rangeSumQuery(tree, 2*index, s, mid, l, r);
    int rightans = rangeSumQuery(tree, 2*index + 1, mid + 1, e, l, r);
